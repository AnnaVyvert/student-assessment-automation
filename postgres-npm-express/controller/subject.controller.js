...
class subject_controller {
    async create(req, res){
        const {name, hours, exam} = req.body
        try{
            const call = await db.query(`INSERT INTO public.subject (name, hours, exam, deleted) 
            values ($1, $2, $3, $4) RETURNING *`,
            [name, hours, exam, false])
            res.json(call.rows[0])
        }catch(e){res.status(400).send(e)}
    }
    async get_all(req, res){
        const call = await db.query(`SELECT *, case exam when true then 'Экзамен' else 'Зачёт' end as exam_label FROM subject where deleted = false order by name`)
