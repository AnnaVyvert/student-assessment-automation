...
class student_controller {
    async create_race(req, res){
        ...
        try{
            ...
            const call = await db.query(`INSERT INTO race (name, sex, distance, age_id, start, deleted, creator_id, competition_id) values ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
    ...
    async get_race(req, res){
        const id = req.params.id
        const call = await db.query(`SELECT * FROM race where id = ${id}`)
    ...
    async get_races(req, res){
        const id = req.params.id
        const call = await db.query(`SELECT * FROM race where competition_id = ${id} and deleted <> true`)
    ...
    async upd_race_parameter(req, res){
        const id = req.params.id
        const column = req.params.column
        const parameter = req.params.parameter
        
        const call_text = `UPDATE race set ${column} = '${parameter}' where id = ${id} RETURNING *`
        console.log(call_text)
        const call = await db.query(call_text)
        res.json(call.rows[0])
    }    
    async del_race(req, res){
        const id = req.params.id
        const dependences = await db.query(`UPDATE athlet set trainer_id = null where trainer_id = ${id}`)
        
        const call = await db.query(`UPDATE race set deleted = true where id = ${id} RETURNING *`)
        res.json(call.rows[0])
    }
}
